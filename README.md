...
- **Junk filter** — drops objects with no `typeKey` and agents whose `system` field is suspiciously short. Set `GLON_WORLD_JUNK_FILTER=0` to disable.
- **Dedupe filter** — collapses identity-duplicate peers and agents (same `display_name + kind + email + discord_id` for peers; same `name` for agents). Keeps the member with the highest `changeCount`. Set `GLON_WORLD_DEDUPE=0` to disable.
...
GLON_WORLD_DEDUPE=0 GLON_WORLD_JUNK_FILTER=0 npm run dev   # raw mode
```
